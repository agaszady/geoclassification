...
warminskie_prng['idZewnetrz'] = pd.to_numeric(warminskie_prng['idZewnetrz'],
                                              errors='coerce').astype('Int64')
warminskie_prng['idMscNd'] = pd.to_numeric(warminskie_prng['idMscNd'],
                                           errors='coerce').astype('Int64')
...
# zmiana nazwy kolumny z idZewnetrz na SIMC_id (dla lepszej czytelności i
# umożliwienia późniejszego łączenia tabel)
...
warminskie_prng = warminskie_prng[~warminskie_prng['rodzaj'].isin(['miasto',
                                                                   'część miasta', 'osiedle'])]
...
warminskie_prng['czy_samodzielna'] = \
    [True if pd.isna(id_msc_nd) else False for id_msc_nd in warminskie_prng['idMscNd']]
...
# zgrupowanie rekordów wg identyfikatora SIMC
# — zmiana z geometrii pojedynczych punktów (Point) na skupiska (MultiPoint)
...
miejscowosci_z_adresacja = (
    pd.merge(punkty_adresowe_simc,
             warminskie_prng[['SIMC_id', 'czy_samodzielna', 'nazwaGlown', 'rodzaj']],
             how='left', on='SIMC_id'))
...
zbior_samodzielny = (
    miejscowosci_z_adresacja)[miejscowosci_z_adresacja['czy_samodzielna'] == True]
zbior_niesamodzielny = (
    miejscowosci_z_adresacja)[miejscowosci_z_adresacja['czy_samodzielna'] == False]
